...
export class GrowTree {
  constructor(scene) {
    this._scene  = scene
    this._models = {}  // element -> normalized THREE.Object3D (not in scene)
    this._state  = {}  // element -> { anchor, growing, growStart, lastProgress, seedTimer }
  }
  ...
  // Place element at a fixed world position and begin seed → grow sequence.
  place(element, worldPosition) {
    if (this._state[element]) return  // already placed
  ...
  // Update all active models. Returns { element: progress (0–1) } for all placed elements.
  update() {
    const result = {}
